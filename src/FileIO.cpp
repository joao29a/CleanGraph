...
void
 FileIO::get_file_contents(const char *filename)
{
    string line;
...
void FileIO::processVetex(GraphManipulator * g)
{
    ...
    for (vector < string >::iterator it = vertex.begin();
	 it != vertex.end(); ++it) {
	g->addVertex(*it);
    }
...
void FileIO::processEdges(GraphManipulator * g, string type)
{
    ...
    for (vector < string >::iterator it = edges.begin(); it != edges.end();
	 ++it) {
...
string
    FileIO::fillStructureWithDataInFile(GraphManipulator * g, string type)
{
    processVetex(g);
    processEdges(g, type);
    return vertex.front();
